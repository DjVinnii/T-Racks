        ...
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    <a href="{{ route('row.create') }}" class="btn btn-app">
    ...
    <script type="text/javascript">
        $(function () {
            $('#datatable').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('row.datatable') !!}',
                columns : [
                    { data: 'name' , name: 'name' },
                    { data: 'location_id' , name: 'location_id' },
                    { data: 'racks' , name: 'racks' },
                    { data: function(e) {
                            let returnval = '';
                            let view = '{{ route('row.show', -1) }}'.replace('-1', e.id);
                            let edit = '{{ route('row.edit', -1) }}'.replace('-1', e.id);
